...
\newcommand{\connection}[1]{\textbf{Connections:} #1}
...
\returns{Tuple of (list of 1D grids, optional Cartesian product array).}

\connection{The 1D grids are used independently in \texttt{interpolation.py}; the Cartesian product is useful for computing expectations over joint distributions.}
...
\textbf{Purpose:} Gauss-Legendre quadrature for integrals over bounded intervals $[a, b]$. Optimal for smooth functions on compact domains.

\textbf{Arguments:}
\begin{itemize}[leftmargin=2cm]
    \item[\texttt{n}] Number of nodes.
