...
\begin{lstlisting}
def qnwcheb(n: int, a: float, b: float, kind: str = "gauss") -> Tuple[np.ndarray, np.ndarray]
\end{lstlisting}

\textbf{Purpose:} Chebyshev-based quadrature on $[a,b]$. Two variants are supported:
\begin{itemize}
    \item \texttt{kind="gauss"} (default): Gauss-Chebyshev with weighting function $w(x)=1/\sqrt{1-x^2}$.
    \item \texttt{kind="clenshaw\_curtis"}: Clenshaw--Curtis weights on Chebyshev nodes for unweighted integrals (CompEcon-compatible).
\end{itemize}
...
\begin{arglist}
    ...
    \item[\texttt{kind}] Either \texttt{"gauss"} or \texttt{"clenshaw\_curtis"}.
\end{arglist}
...
\begin{itemize}
    ...
    \item For integrals with weight function $1/\sqrt{1-x^2}$ (\texttt{kind="gauss"}).
    \item For unweighted integrals using Chebyshev nodes (\texttt{kind="clenshaw\_curtis"}).
...
\subsubsection{Chebyshev Approximation in Practice (Problem Set 1)}

For approximating nonlinear functions (e.g., value functions or Runge's function), compare accuracy across polynomial degrees. Using \textbf{Chebyshev nodes} avoids the Runge phenomenon that occurs with equidistant nodes.

\paragraph{Example: Runge's function.} Approximate $f(x) = 1/(1+x^2)$ on $[-10, 10]$ (a classic test where equidistant polynomial interpolation diverges near the boundaries). With Chebyshev approximation, error decreases as $n$ increases.

\begin{lstlisting}
from hetmacro.interpolation import cheb_nodes, cheb_coef, cheb_eval
import numpy as np
...
# Compare polynomial degrees 5, 10, 15, 20
for n in [5, 10, 15, 20]:
    coef = cheb_coef(runge, n=n, a=a, b=b)
    approx = cheb_eval(coef, x_fine, a=a, b=b)
    err = np.max(np.abs(approx - f_true))
    print(f"n={n}: max error = {err:.2e}")
\end{lstlisting}

\paragraph{Visualizing approximation error.} Plot \texttt{approx} and \texttt{f\_true} on \texttt{x\_fine} for each $n$ to see how the error concentrates near the boundaries when $n$ is small and decreases globally as $n$ increases. For theoretical error bounds (Rivlin's theorem), see the Macro Bible Appendix B --- Function Approximation.
...
\subsection{Basis Matrix Helpers (Spline Collocation)}

\begin{lstlisting}
def spline_basis_matrix(
...
\textbf{Purpose:}
\begin{itemize}
    \item \texttt{spline\_basis\_matrix}: sparse B-spline design matrix at evaluation points.
    \item \texttt{cubic\_basis\_matrix}: cubic basis ($k=3$), used for value-function collocation.
    \item \texttt{linear\_basis\_matrix}: linear tent basis ($k=1$), used for distribution transition matrices.
    \item \texttt{tensor\_basis\_matrix}: row-wise Kronecker product for 2D tensor-product collocation.
\end{itemize}
...
\paragraph{Rigorous treatment.}
A macro-level, rigorous treatment of calibration (method of simulated moments), root finding (bisection, Brent, Newton, Broyden), optimization (golden section, Nelder--Mead, quasi-Newton), and Euler equation methods, with worked examples and figures from Problem Set 1, is given in the \textbf{Macro Bible} (Lecture Notes in Macroeconomics), Appendix B --- Numerical Methods chapter (\texttt{Parts\_and\_chapters/Appendix/numerical\_methods/ch\_numerical\_methods.tex}).
...
\subsubsection{\texttt{broyden} (unified Broyden solver)}
\begin{lstlisting}
def broyden(
    f: Callable,
    x0: np.ndarray,
...
\textbf{Purpose:} Single entry point for Broyden-style solvers. Solves $F(x) = 0$ for systems of nonlinear equations. The argument \texttt{method} selects the implementation.

\textbf{Arguments:}
\begin{arglist}
    \item[\texttt{method}] One of \texttt{"scipy"}, \texttt{"compecon"}, or \texttt{"componentwise"}.
    \item[\texttt{a, b}] Required when \texttt{method="componentwise"}: lower and upper bounds on $x$.
    \item[\texttt{**kwargs}] Passed to the underlying solver (e.g., \texttt{max\_steps}, \texttt{show\_iters} for \texttt{"compecon"}).
\end{arglist}

\textbf{When to use which method:}
\begin{itemize}
    \item \oldtextbf{\texttt{method="compecon"}}: Full coupled system $F(x)=0$ (e.g., equilibrium residuals in all $n$ unknowns). CompEcon-style: inverse Jacobian, backstepping line search, optional restarts. Use when the system is nonlinear or ill-scaled; SciPy's Broyden can diverge in such cases.
    \item \oldtextbf{\texttt{method="scipy"}}: Same full-system case when the problem is well-behaved. Thin wrapper around \texttt{scipy.optimize.root(..., method="broyden1")}.
    \item \oldtextbf{\texttt{method="componentwise"}}: Vectorized, largely \emph{componentwise} equations with bounds $a \le x \le b$ (e.g., many independent 1D FOCs). Requires \texttt{a} and \texttt{b}. Use for the inner step of a fixed-point iteration (e.g., household FOCs for hours given wages).
\end{itemize}

\textbf{Backward compatibility:} \texttt{broyden\_compecon(...)} and \texttt{solve\_broyden(func, x, a, b, ...)} are thin wrappers that call \texttt{broyden} with the appropriate \texttt{method}.
...
\subsubsection{\texttt{golden\_max\_vec}}
\begin{lstlisting}
def golden_max_vec(
    f: Callable,
    a: np.ndarray | float,
...
\textbf{Purpose:} Vectorized golden-section \emph{maximization} on per-element intervals. Accepts arrays \texttt{a}, \texttt{b} (bounds for each element) and a vectorized objective \texttt{f}; returns maximizers \texttt{x} and \texttt{f(x)}.

\textbf{When to use:} Many independent 1D maximization problems (e.g.\ Bellman policy step over a grid) without Python loops.
...
\textbf{Purpose:} Nelder-Mead simplex algorithm for derivative-free multi-dimensional optimization (analogous to MATLAB's \texttt{fminsearch}).

\returns{If \texttt{return\_info=False}: tuple \texttt{(x\_opt, f\_opt)}. If \texttt{return\_info=True}: \texttt{(x\_opt, f\_opt, hist)} where \texttt{hist} has keys \texttt{"x"} (list of parameter vectors) and \texttt{"f"} (list of objective values at each iteration), so you can plot the error/SSE trace. Optional \texttt{callback(xk)} is invoked each iteration; \texttt{options} are passed to \texttt{scipy.optimize.minimize} (e.g.\ \texttt{maxiter}, \texttt{disp}).}
...
\begin{tabular}{llll}
\toprule
\oldtextbf{Method} & \oldtextbf{Dimension} & \oldtextbf{Requirements} & \oldtextbf{Best For} \\
\midrule
\texttt{bisect} & 1D & Bracketed root & Simple, guaranteed convergence \\
\texttt{brentq} & 1D & Bracketed root & Default 1D choice; superlinear \\
\texttt{newton} & 1D & Derivative $f'(x)$ & Fast convergence near root \\
\texttt{secant} & 1D & None & Derivative-free Newton approx. \\
\texttt{broyden} & $n$D & Initial guess & Systems of equations $F(x)=0$ \\
\bottomrule
\end{tabular}
\end{center}
...
\oldtextbf{Method} & \oldtextbf{Dimension} & \oldtextbf{Requirements} & \oldtextbf{Best For} \\
\midrule
\texttt{golden\_search} & 1D & Bounds $[a,b]$ & Unimodal, no derivatives \\
\texttt{brent\_min} & 1D & Bounds $[a,b]$ & Default 1D minimization \\
\texttt{nelder\_mead} & $n$D & Initial guess & Derivative-free, noisy objectives \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
    \item \oldtextbf{Root-finding or optimization?}
    \begin{itemize}
        \item Root-finding: solving $f(x)=0$ (e.g., market clearing, Euler equations)
        \item Optimization: minimizing $f(x)$ (e.g., calibration, moment matching)
    \end{itemize}
    \item \oldtextbf{One dimension or many?}
    \begin{itemize}
        \item 1D root: use \texttt{brentq} (bracketed) or \texttt{newton} (with derivative)
        \item 1D minimum: use \texttt{brent\_min}
        \item $n$D root: use \texttt{broyden} with \texttt{method="compecon"}
        \item $n$D minimum: use \texttt{nelder\_mead}
    \end{itemize}
    \item \oldtextbf{Is the objective noisy or simulation-based?}
    \begin{itemize}
        \item Yes: use \texttt{nelder\_mead} (robust to noise, no gradients needed)
        \item No: consider gradient-based methods if available
    \end{itemize}
\end{enumerate}

\textbf{Example: Market Clearing vs Calibration}
\begin{lstlisting}
from hetmacro.optimize import brentq, nelder_mead

# Root-finding: find r* such that A(r) - K(r) = 0
r_star = brentq(lambda r: A(r) - K(r), a=0.01, b=0.05)

# Optimization: find params minimizing SSE of moments
def sse(params):
    model_moments = simulate_model(params)
    return np.sum((model_moments - target_moments)**2)

params_opt, sse_opt = nelder_mead(sse, x0=np.array([0.9, 0.1, 0.05]))
\end{lstlisting}

\subsection{Calibration by Simulation (Method of Simulated Moments)}

Many macroeconomic models require calibrating parameters to match empirical moments. When analytical moments are unavailable, we use \oldtextbf{simulation-based calibration}: simulate the model, compute moments from simulated data, and minimize the distance to target moments.

\subsubsection{Workflow}

\begin{enumerate}
    \item \oldtextbf{Define target moments} from empirical data (e.g., standard deviation of log income, autocorrelations).
    \item \oldtextbf{Write a simulation function} that takes parameters and returns model-implied moments.
    \item \oldtextbf{Construct a loss function} measuring distance between model and target moments (typically sum of squared errors, SSE).
    \item \oldtextbf{Optimize} using \texttt{nelder\_mead} (derivative-free, robust to simulation noise).
\end{enumerate}

\subsubsection{Example: Calibrating an Income Process}
...
def simulate_income(rho, sigma_u, sigma_eps, T=10000, seed=42):
    """Simulate observed log income with measurement error."""
    np.random.seed(seed)
    logy = np.zeros(T)
    for t in range(1, T):
        logy[t] = rho * logy[t-1] + sigma_u * np.random.randn()
    logy_obs = logy + sigma_eps * np.random.randn(T)
    return logy_obs
...
def sse_loss(params):
    ...
    sse = sum((model[k] - target[k])**2 for k in target)
    return sse
...
print(f"Optimal params: rho={params_opt[0]:.4f}, "
      f"sigma_u={params_opt[1]:.4f}, sigma_eps={params_opt[2]:.4f}")
print(f"Final SSE: {sse_opt:.6f}")
\end{lstlisting}

\subsubsection{Convergence Diagnostics}

When using \texttt{nelder\_mead} with \texttt{return\_info=True}, you receive a history dictionary for diagnostics:
...
\begin{itemize}
    \item \oldtextbf{Multiple restarts}: Run from several initial guesses to avoid local minima.
    \item \oldtextbf{Check final SSE}: A very small SSE suggests good fit; a large SSE may indicate model misspecification.
    \item \oldtextbf{Examine moment fit}: Compare model and target moments individually to identify which are hardest to match.
    \item \oldtextbf{Increase simulation length}: Longer simulations reduce Monte Carlo noise in moment estimates.
\end{itemize}

\paragraph{Appendix B (Macro Bible).}
For the MSM objective, weight matrix choice, stochastic process (AR(1)+noise and jump-diffusion), and figures from the income calibration problem set, see the Macro Bible (Lecture Notes in Macroeconomics), Appendix B --- Numerical Methods, section ``Calibration and Estimation''.
...
\subsection{Function: \texttt{stationary\_eigenvector}}

\begin{lstlisting}
def stationary_eigenvector(Q, tol: float = 1e-12) -> np.ndarray
\end{lstlisting}

\textbf{Purpose:} Recover stationary distribution as the left unit-eigenvector of a transition matrix:
\[
...
\subsection{Function: \texttt{solve\_aiyagari\_ge}}
...
\textbf{Purpose:} Solve the Aiyagari model GE steady state with any solver backend.

\textbf{Workflow:}
\begin{enumerate}
    \item Accept a solver object (EGM, GridVFI, PFI, HowardGrid, etc.)
    \item For each candidate $r$: compute prices, build \texttt{Household}, solve, compute ergodic distribution and aggregates
    \item Find market-clearing $r^*$ using Brent (\texttt{method="brentq"}) or bisection (\texttt{method="bisection"})
\end{enumerate}

\textbf{Returns:} Dictionary with \texttt{r, K, w, Y, C, household, solved\_policy, distribution, aggregates, solver\_name, method, n\_iterations, elapsed}.

\subsection{Function: \texttt{capital\_supply\_curve}}

Evaluates household capital supply $K_s(r)$ on a grid of interest rates for plotting supply vs demand curves.

\textbf{Example:}
\begin{lstlisting}
from hetmacro.models.aiyagari import solve_aiyagari_ge, capital_supply_curve
...
# Solve GE
ss = solve_aiyagari_ge(
    EGM(), income, a_grid,
    beta=0.96, gamma=1.0, verbose=True
)
print(f"r* = {ss['r']:.4f}, K* = {ss['K']:.2f}")
...
% ==================================================================
\chapter{Worked Examples from Problem Sets}
% ==================================================================

This chapter summarizes how the tools in \texttt{hetmacro} combine to solve the main numerical tasks in Problem Set 1: root finding for equilibrium, calibration by simulation, dynamic programming, and Euler equation methods. Each section gives the economic setup, the relevant \texttt{hetmacro} functions, and a compact workflow. For full theory, see the Macro Bible (Lecture Notes in Macroeconomics), Appendix B --- Numerical Methods.

\section{Root Finding for Market Clearing (Problem 2)}

\paragraph{Setup.} Static labor supply equilibrium with heterogeneous ability $e$: households choose hours $h(e)$; production $Y = L^\eta$; wage $W = \eta L^{\eta-1}$; government rebates tax revenue lump-sum. Equilibrium is a vector $\mathbf{h}$ such that each household's FOC holds and the labor market clears. This reduces to solving $F(\mathbf{h}) = \mathbf{0}$ where $F_i$ is the residual of the $i$-th household's first-order condition.

\paragraph{Method.} Use \texttt{broyden} with \texttt{method='compecon'} for the full coupled system. Alternative: fixed-point iteration on the wage $W$ with an inner \texttt{broyden(..., method='componentwise', a=0, b=}\texttt{hmax)} for the vector of hours given $W$.

\paragraph{Code pattern.}

\begin{lstlisting}
from hetmacro.optimize import broyden
import numpy as np

def residual(h, omega, e, eta, tau, sigma, gamma):
    """FOC residuals: h_i^gamma - (1-tau)*W*e_i*c_i^{-sigma}."""
    L = np.dot(omega, e * h)
    W = eta * L**(eta - 1)
    T = L**eta - (1 - tau) * W * L
    c = T + (1 - tau) * W * e * h
    return h**gamma - (1 - tau) * W * e * (c**(-sigma))

h0 = np.full(n, 0.3)  # initial guess
h_star, info = broyden(lambda h: residual(h, omega, e, eta, tau, sigma, gamma),
                        h0, method='compecon', tol=1e-10)
\end{lstlisting}

\paragraph{Convergence.} Plot $\|F(\mathbf{h}^{(k)})\|$ vs.\ iteration to verify convergence. See codebook \texttt{optimize.py} and Macro Bible Appendix B for Broyden theory and labor market derivation.

\section{Calibration by Simulation (Problem 3)}

\paragraph{Setup.} Calibrate an AR(1) process with measurement error for log productivity: $y_t = x_t + \varepsilon_t$, $x_t = \rho x_{t-1} + u_t$. Target moments: sd($\log y$), sd($\Delta \log y$), autocorrelations at lags 1, 3, 5.

\paragraph{Method.} Simulate a panel, compute model moments, minimize sum of squared errors (SSE) between model and target moments using \texttt{nelder\_mead}. Extension: jump-diffusion for fat tails (see Calibration Workflows in \texttt{macro\_agents.md}).

\noindent \textbf{Code pattern.}
\begin{lstlisting}
from hetmacro.optimize import nelder_mead
import numpy as np
...
def sse_loss(params):
    rho, sigma_u, sigma_eps = params
    if not (0 < rho < 1 and sigma_u > 0 and sigma_eps > 0):
        return 1e10
    logy = simulate_ar1_noise(rho, sigma_u, sigma_eps, T=10000)
    model = compute_moments(logy)  # sd, acf1, acf3, acf5
    return np.sum((np.array([model['sd'], model['acf1'], ...]) - target)**2)

params_opt, sse_opt, hist = nelder_mead(sse_loss, x0=np.array([0.9, 0.2, 0.1]),
                                         return_info=True)
# Plot hist['f'] for SSE convergence
\end{lstlisting}

See \texttt{optimize.py} ``Calibration by Simulation'' and Macro Bible ``Calibration and Estimation'' for MSM and jump-diffusion formulas.

\section{Dynamic Programming (Problem 4)}

\paragraph{Setup.} Two-period or multi-period consumption-savings with income risk. Solve for value function $V(a,e)$ and policy $a'(a,e)$ (and consumption $c(a,e)$) via Bellman iteration.

\paragraph{Method.} Use \texttt{backward.policy\_iteration} with \texttt{method='vfi'} (value function iteration) or \texttt{method='egm'} (endogenous grid). Grids from \texttt{grids.py}; income process from \texttt{markov.rouwenhorst}; expectations via \texttt{quadrature} or the Markov chain.
...
y = np.exp(e)
Va, a_pol, c_pol = policy_iteration(Pi, a_grid, y, r=0.04, beta=0.96,
                                     eis=0.5, method='vfi')
\end{lstlisting}

Policy functions are on the grid; use \texttt{interp\_linear} to evaluate at off-grid points. For projection/collocation methods, keep \textbf{solve grid} and \textbf{output/evaluation grid} separate: solve on basis nodes (Greville/Chebyshev), then evaluate policies on a common fine grid before distribution iteration and method comparison.

\section{Euler Equation Methods (Problem 5)}

\paragraph{Setup.} Portfolio choice or consumption-savings where the solution is characterized by Euler equations. At an interior optimum, $u'(c_t) = \beta (1+r) \mathbb{E}[u'(c_{t+1})]$ (and similarly for risky asset). Solve by time iteration: guess tomorrow's policy, solve today's from the Euler equation, repeat.

\paragraph{Method.} For each state $(a,e)$ on the grid, solve the nonlinear equation (Euler residual = 0) for today's choice; use \texttt{broyden} or \texttt{brentq} for the inner 1D solve. Quadrature (\texttt{quadrature.qnwnorm} or \texttt{qnwnorm\_mv}) for expectations. Check accuracy with the \textbf{Euler residual}: left-hand side minus right-hand side of the Euler equation; should be small in the interior.

\noindent \textbf{Code pattern.}
\begin{lstlisting}
from hetmacro.quadrature import qnwnorm_mv
from hetmacro.optimize import broyden  # or brentq for 1D

# For each (a, e): find x, x_s that satisfy Euler equations
# res_safe = 1 - beta * E[M2 * (1+r)]
# res_risky = 1 - beta * E[M2 * (1+r+eps)]
nodes, weights = qnwnorm_mv(n=[7, 7], mu=mu, Sigma=Sigma)
# ... define residual function, then broyden(residual, x0)
\end{lstlisting}
...
\section{Public API Sync Checklist}

This checklist is the compact, module-level API reference for agents and users. The rule is simple: if a public function in \texttt{hetmacro/*.py} changes, update this section and any relevant narrative sections before rebuilding \texttt{codebook.pdf}.

\subsection{Foundational Modules}
\begin{itemize}
    \item \texttt{grids.py}: \texttt{make\_asset\_grid}, \texttt{power\_grid}, \texttt{make\_grid}, \texttt{double\_exponential\_grid}, \texttt{log\_spaced\_grid}, \texttt{make\_grid\_1d}, \texttt{make\_grid\_nd}, \texttt{cartesian\_product}, \texttt{gridmake}, \texttt{tensor\_product}, \texttt{smolyak\_sparse\_grid}
    \item \texttt{quadrature.py}: \texttt{qnwlege}, \texttt{qnwcheb}, \texttt{qnwnorm} (\texttt{sigma} or \texttt{sig2}), \texttt{qnwlogn}, \texttt{qnwunif}, \texttt{qnwsimp}, \texttt{qnwtrap}, \texttt{qnwbeta}, \texttt{qnwgamma}, \texttt{qnwnorm\_mv}
    \item \texttt{interpolation.py}: \texttt{FunctionSpace}, \texttt{interp\_linear}, \texttt{interp\_bilinear}, \texttt{get\_lottery}, \texttt{spline\_fit}, \texttt{spline\_coef}, \texttt{spline\_eval}, \texttt{spline\_basis\_matrix}, \texttt{cubic\_basis\_matrix}, \texttt{linear\_basis\_matrix}, \texttt{tensor\_basis\_matrix}, \texttt{cheb\_nodes}, \texttt{cheb\_basis}, \texttt{cheb\_coef}, \texttt{cheb\_eval}
    \item \texttt{optimize.py}: \texttt{bisect}, \texttt{brentq}, \texttt{newton}, \texttt{secant}, \texttt{broyden}, \texttt{broyden\_compecon}, \texttt{solve\_broyden}, \texttt{golden\_search}, \texttt{golden\_max\_vec}, \texttt{brent\_min}, \texttt{nelder\_mead}, \texttt{minimize\_lbfgsb}
    \item \texttt{markov.py}: \texttt{rouwenhorst}, \texttt{tauchen}, \texttt{stationary\_distribution}, \texttt{simulate\_markov}, \texttt{simulate\_mc}, \texttt{markov\_chain\_ar1}
    \item \texttt{utils.py}: \texttt{crra\_utility}, \texttt{crra\_marginal}, \texttt{crra\_inverse\_marginal}, \texttt{ces\_aggregator}, \texttt{compute\_fixed\_point}, \texttt{numerical\_derivative}, \texttt{numerical\_jacobian}, \texttt{tic}, \texttt{toc}
\end{itemize}

\subsection{Heterogeneous-Agent Modules}
\begin{itemize}
    \item \texttt{backward.py}: \texttt{backward\_egm}, \texttt{backward\_vfi}, \texttt{policy\_iteration}, \texttt{policy\_evaluation\_discrete}, \texttt{howard\_policy\_evaluation}, \texttt{coefficient\_vfi\_step}
    \item \texttt{forward.py}: \texttt{forward\_iteration}, \texttt{stationary\_distribution}, \texttt{expectation\_iteration}, \texttt{expectation\_functions}, \texttt{stationary\_markov}, \texttt{stationary\_eigenvector}, \texttt{update\_policy\_1a}, \texttt{update\_exogenous\_1a}
    \item \texttt{steady\_state.py}: \texttt{household\_steady\_state}, \texttt{market\_clearing}
    \item \texttt{distributions.py}: \texttt{forward\_policy\_step}, \texttt{stationary\_distribution\_ha}, \texttt{lottery\_indices}, \texttt{compute\_joint\_transition\_matrix}, \texttt{stationary\_dist}
    \item \texttt{dp\_tools.py}: \texttt{solve\_policy\_egm}, \texttt{solve\_policy\_vfi}, \texttt{solve\_steady\_policy}, \texttt{interp\_grid}, \texttt{solve\_euler\_one\_asset}
    \item \texttt{income\_process.py}: \texttt{DiscreteIncome}, \texttt{RouwenhorstIncome}, \texttt{TauchenIncome}, \texttt{ContinuousQuadratureIncome}
    \item \texttt{household.py}: \texttt{SolvedPolicy}, \texttt{Household}
    \item \texttt{solvers/*}: \texttt{GridVFI}, \texttt{HowardGrid}, \texttt{HowardImprovement}, \texttt{PolicyFunctionIteration}, \texttt{EGM}, \texttt{CollocationVFI\_Spline}, \texttt{CollocationVFI\_Chebyshev}, \texttt{NaiveEulerIteration}, \texttt{HowardEulerIteration}
    \item \texttt{ssj.py}: \texttt{jacobian}, \texttt{step1\_backward}, \texttt{J\_from\_F}
    \item \texttt{models/aiyagari.py}: \texttt{firm\_k\_demand}, \texttt{firm\_wage}, \texttt{capital\_supply\_curve}, \texttt{solve\_aiyagari\_ge}
\end{itemize}
